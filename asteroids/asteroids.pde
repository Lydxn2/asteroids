...
PFont deltaFont, deltaBFont;
GIF[] heartGif;
AnimatedText titleText, toGameText, youLoseText, youWinText, toStartText;

boolean keyLeft, keyRight, keyUp, keySpace, keyEnter;
...
void setup() {
  ...
  heartGif = new GIF[3];
  for (int i = 0; i < 3; i++)
    heartGif[i] = new GIF("data/images/heart", 2, 5);
...
void resetGame() {
  ...
  youWinText = new FloatingText("You WIN!", deltaBFont, width / 2, height / 2, 100, #FF0000);
